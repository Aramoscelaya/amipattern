...
export default function InventoryModal({ visible, initial, onClose, onSave }) {
  ...
  // Cuando cambia subtipo de material → ajusta unidad automáticamente
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ overflowY:'auto', flex:1, padding:'4px 20px 20px' }}>
          ...
          <div style={{ display:'flex', gap:12 }}>
            <div style={{ flex:1 }}>
              <Label t={`Stock total (${unidadLabel})`} />
              <Input type="number" value={f.stock_total}
                onChange={e=>upd('stock_total',e.target.value)}
                placeholder={f.unidad==='g' ? 'ej: 200' : 'ej: 50'}/>
            </div>
            <div style={{ flex:1 }}>
              <Label t={`Alerta si quedan menos de (${unidadLabel})`} />
              <Input type="number" value={f.alerta_minimo}
          ...
          <Input type="number" value={f.costo_unitario}
            ...
            placeholder="ej: 0.15"/>
          {f.costo_unitario > 0 && f.stock_total > 0 && (
            <div style={{ fontSize:12, color:'#6B7280', marginTop:4 }}>
              💰 Valor total del stock: <strong style={{color:'#1A1A2E'}}>
                ${(f.costo_unitario * f.stock_total).toFixed(2)}
              </strong>
            </div>
          )}

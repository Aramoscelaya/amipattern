...
export default function InventoryScreen({ user }) {
  const { items, loading, error, saveItem, deleteItem, stockBajoItems } = useInventory(user?.id);
  ...
  // ── Notificación push cuando hay stock bajo ──
  useEffect(() => {
  ...
  const filtered = items.filter(item => {
    ...
    if (filtro === 'alerta') {
      const disp = item.stock_total - item.stock_usado;
  ...
  const valorTotal    = items.reduce((sum, i) => {
    const disp = i.stock_total - i.stock_usado;
    return sum + (disp * (i.costo_unitario || 0));
  }, 0);
